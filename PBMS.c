...
//      contact.CreateTime = time_str;
	  ...
	  while(strlen(contact.PhoneNumber)>11){
	  	printf("\t\t\tPhoneNumber can\'t be more than 11 digits.");
	  	printf("\n\t\t\tPhoneNumber: ");
	    gets(contact.PhoneNumber);	
	  }
...
void showContacts(int i, struct Contact contact){
	...
	printf("\t\t\tNotes: %s\n",contact.Notes);
//			printf("\t\t\tIsBlocked: %s\n",contact.IsBlocked==True?"True":"False");
}
...
//	printf("\t\tId\tName\tPhoneNumber\tEmail\t\tGroup\tCompany\t\tAddress\t\tNotes\tIsBlocked\n");
//	printf("\t\t----------------------------------------------------------------------------------\n");
...
//	 	printf("\t\t%d\t%s\t%s\t%s\t%d\t%s\t%s\t%s",contact.Id,contact.Name,contact.PhoneNumber,contact.Email,contact.Group,contact.Company,contact.Address,contact.Notes,contact.IsBlocked==True?"True":"False");
	}
	Total = i;
	if(K<1 && hide==0){
...
void findBy(int id, char* key){
	...
	for(i=0;i<K;i++){
		if(id==1){
			...
			if(strstr(contacts[i].PhoneNumber, key)){
				result[k++] = contacts[i];	
			}
		}else if(id==3){
